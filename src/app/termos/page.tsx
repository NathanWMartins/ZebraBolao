...
function Section({ number, title, children }: { number: string; title: string; children: React.ReactNode }) {
    return (
        <Box sx={{ mb: 5 }}>
            <Typography sx={{ fontSize: { xs: 17, md: 19 }, fontWeight: 600, color: '#C9940A', mb: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
                <Box component="span" sx={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 28, height: 28, borderRadius: '50%', bgcolor: 'rgba(201,148,10,0.12)', border: '0.5px solid rgba(201,148,10,0.3)', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                    {number}
                </Box>
                {title}
            </Typography>
            <Box sx={{ pl: { xs: 0, md: 5 } }}>{children}</Box>
        </Box>
    )
}

function P({ children }: { children: React.ReactNode }) {
    return <Typography sx={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', lineHeight: 1.8, mb: 1.5 }}>{children}</Typography>
}

function SubTitle({ children }: { children: React.ReactNode }) {
    return <Typography sx={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.85)', mt: 2, mb: 1 }}>{children}</Typography>
}
...
function BulletList({ items }: { items: string[] }) {
    return (
        <Box component="ul" sx={{ pl: 2.5, mt: 0.5, mb: 1.5 }}>
            {items.map((item, i) => (
                <Box component="li" key={i} sx={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', lineHeight: 1.8, mb: 0.5, '&::marker': { color: '#C9940A' } }}>
                    {item}
                </Box>
            ))}
        </Box>
...
function ExternalLink({ href, children }: { href: string; children: React.ReactNode }) {
    return (
        <Box component="a" href={href} target="_blank" rel="noopener noreferrer" sx={{ color: '#C9940A', textDecoration: 'none', '&:hover': { textDecoration: 'underline' } }}>
            {children}
        </Box>
    )
}
...
export default function TermosPage() {
    return (
        <Box sx={{ minHeight: '100vh', bgcolor: '#111110', backgroundImage: 'repeating-linear-gradient(-55deg, transparent, transparent 18px, rgba(255,255,255,0.03) 18px, rgba(255,255,255,0.03) 36px)' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: 'rgba(0,0,0,0.7)', px: 3, py: 0.5, borderBottom: '0.5px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 50 }}>
                <Link href="/" style={{ textDecoration: 'none' }}>
                    <Image src="/LogoZebra.png" alt="Zebra Bolão" width={140} height={60} style={{ objectFit: 'contain' }} />
                </Link>
                <Typography sx={{ color: 'rgba(255,255,255,0.4)', fontSize: 13 }}>Copa 2026</Typography>
            </Box>

            <Container maxWidth="md" sx={{ py: { xs: 5, md: 8 }, px: { xs: 3, md: 4 } }}>
                <Box sx={{ mb: 6 }}>
                    <Box sx={{ display: 'inline-block', bgcolor: 'rgba(201,148,10,0.12)', border: '0.5px solid rgba(201,148,10,0.3)', borderRadius: '20px', px: 2, py: 0.6, mb: 3 }}>
                        <Typography sx={{ fontSize: 12, color: '#C9940A' }}>Documento legal</Typography>
                    </Box>
                    <Typography variant="h1" sx={{ fontSize: { xs: 28, md: 38 }, fontWeight: 600, color: '#fff', letterSpacing: -0.5, mb: 1.5 }}>
                        Termos de Uso
                    </Typography>
                    <Typography sx={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', mb: 4 }}>Última atualização: {today}</Typography>
                    <Box sx={{ bgcolor: 'rgba(201,148,10,0.07)', border: '0.5px solid rgba(201,148,10,0.2)', borderRadius: '10px', p: 2.5 }}>
                ...
                <Section number="1" title="Sobre o Serviço">
                    ...
                    <P>
                        ...
                        sobre os resultados das partidas, competindo em rankings individuais e por grupo.
                    </P>
                    <P>O Zebra Bolão não intermediamos, recebemos, gerenciamos nem distribuímos quaisquer valores financeiros entre os participantes.</P>
                </Section>
                ...
                <Section number="2" title="Elegibilidade">
                    ...
                    <BulletList items={[
                        ...
                        'Possuir uma conta válida (Google ou e-mail/senha) para autenticação',
                    ...
                    <P>Ao criar uma conta, você declara que atende a todos esses requisitos.</P>
                </Section>

                <Section number="3" title="Natureza Recreativa e Responsabilidade Financeira">
                    <P>O Zebra Bolão é uma plataforma estritamente recreativa. É fundamental que você compreenda:</P>
                    ...
                    <P>
                        O usuário que cria um grupo é o único responsável pela organização, coleta e distribuição
                        de eventuais prêmios entre os participantes. O Zebra Bolão não possui qualquer envolvimento nessa relação.
                    </P>
                    <SubTitle>c) Isenção de responsabilidade</SubTitle>
                    <P>
                        O Zebra Bolão não se responsabiliza por disputas, inadimplência ou conflitos financeiros
                ...
                <Section number="4" title="Regras dos Palpites">
                    ...
                    <BulletList items={[
                        ...
                        'Prorrogação e pênaltis não são considerados — apenas o tempo regulamentar',
                    ]} />
                </Section>
                ...
                <Section number="5" title="Pontuação e Ranking">
                    <BulletList items={[
                        ...
                        'O ranking é calculado com base no total de acertos ao longo do torneio',
                        ...
                        'Resultados e rankings são atualizados automaticamente após a confirmação do resultado oficial',
                    ]} />
                </Section>
                ...
                <Section number="6" title="Conduta do Usuário">
                    ...
                    <P>O descumprimento pode resultar na suspensão ou exclusão permanente da sua conta, sem aviso prévio.</P>
                </Section>

                <Section number="7" title="Disponibilidade do Serviço">
                    <P>O Zebra Bolão é oferecido gratuitamente e sem garantia de disponibilidade contínua. Nos reservamos o direito de:</P>
                    <BulletList items={[
                ...
                <Section number="8" title="Limitação de Responsabilidade">
                    <P>Na máxima extensão permitida pela legislação brasileira, o Zebra Bolão e seu operador não são responsáveis por:</P>
                    <BulletList items={[
                ...
                <Section number="9" title="Propriedade Intelectual">
                    ...
                    <P>É proibida a reprodução, distribuição, modificação ou uso comercial sem autorização prévia e expressa por escrito.</P>
                </Section>
                ...
                <Section number="12" title="Contato">
                    ...
                    <Box sx={{ bgcolor: 'rgba(255,255,255,0.04)', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: '10px', p: 3, mt: 2 }}>
                        <Typography sx={{ color: '#fff', fontWeight: 600, fontSize: 15, mb: 0.5 }}>Nathan Will Martins</Typography>
                        <Typography sx={{ fontSize: 14, color: 'rgba(255,255,255,0.5)', mb: 0.5 }}>
                            E-mail:{' '}
                            <ExternalLink href="mailto:nathanwillmartins@gmail.com">nathanwillmartins@gmail.com</ExternalLink>
                        </Typography>
                        <Typography sx={{ fontSize: 13, color: 'rgba(255,255,255,0.35)' }}>Prazo de resposta: até 15 dias úteis</Typography>
                    </Box>
                </Section>
